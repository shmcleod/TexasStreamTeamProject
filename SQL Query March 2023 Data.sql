...
SELECT * FROM TST..Datamap
WHERE [Conductivity (탎 cm)] <> 'Data error'
AND pH <> 'Data error'
AND [Dissolved Oxygent (mg L)] <> 'Data error'
AND [Air Temperature (캜)] <> 'Data error'
AND [Water Temperature (캜)] <> 'Data error'
...
OR [Air Temperature (캜)] = 'Data error'
OR [Water Temperature (캜)] = 'Data error'
...
[Conductivity (탎 cm)],
AVG(NULLIF(CAST([Conductivity (탎 cm)] AS float),0)) OVER (PARTITION BY Basin ORDER BY sample_date, Basin) as Cond_Average,
pH,
...
[Air Temperature (캜)],
AVG(NULLIF(CAST([Air Temperature (캜)] AS float),0)) OVER (PARTITION BY Basin ORDER BY sample_date, Basin) as Air_temp_Average,
[Water Temperature (캜)],
AVG(NULLIF(CAST([Water Temperature (캜)] AS float),0)) OVER (PARTITION BY Basin ORDER BY sample_date, Basin) as Water_temp_Average,
[E  Coli Average],
...
[Conductivity (탎 cm)],
AVG(NULLIF(CAST([Conductivity (탎 cm)] AS float),0)) OVER (PARTITION BY site_id ORDER BY sample_date, site_id) as Cond_Average,
pH,
...
[Air Temperature (캜)],
AVG(NULLIF(CAST([Air Temperature (캜)] AS float),0)) OVER (PARTITION BY site_id  ORDER BY sample_date, site_id) as Air_temp_Average,
[Water Temperature (캜)],
AVG(NULLIF(CAST([Water Temperature (캜)] AS float),0)) OVER (PARTITION BY site_id  ORDER BY sample_date, site_id) as Water_temp_Average,
[E  Coli Average],
...
AND [E  Coli Average] <> 'Data error'
AND sample_date <> '1900-01-01'
AND [Conductivity (탎 cm)] IS NOT NULL
